...
o	Assign an instance attribute session_duration:
...
o	Create a Session ID by calling super() - super() will call the create_session() method of SessionAuth
o	Return None if super() can’t create a Session ID
o	Use this Session ID as key of the dictionary user_id_by_session_id - the value for this key must be a dictionary (called “session dictionary”):
	The key user_id must be set to the variable user_id
	The key created_at must be set to the current datetime - you must use datetime.now()
o	Return the Session ID created
•	Overload def user_id_for_session_id(self, session_id=None):
o	Return None if session_id is None
o	Return None if user_id_by_session_id doesn’t contain any key equals to session_id
o	Return the user_id key from the session dictionary if self.session_duration is equal or under 0
o	Return None if session dictionary doesn’t contain a key created_at
o	Return None if the created_at + session_duration seconds are before the current datetime. datetime - timedelta
o	Otherwise, return user_id from the session dictionary
